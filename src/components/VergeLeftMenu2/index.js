...
function VergeLi({ number, text, type, activeMenu, step, menu, onNext, imagenes = null, nextStep = false }) {
  ...
  return (
    <Li canHover={canHover()} className={`${type === "section" && "section"}`} onClick={(handleClick(number))}>
  ...
  const renderView = useCallback((button) => {
    if (button.steps) {
      ...
      return (
        <ul>
          {button.steps.map((i) => (
              <Li key={i.step} className={`${toggleActiveStyles(i.step, i.type)}`} onClick={e => toggleActive(e, i.step)}>
